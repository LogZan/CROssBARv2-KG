...
class UniprotSwissprot:
    ...
    def __init__(
        ...
        self._annotations_file_handle = None
        self._features_file_handle = None
        
        if self.low_memory_mode:
            logger.info("Low memory mode enabled - annotations and features will be streamed to temp files")
            # Create temp directory for intermediate files
            self._temp_dir = tempfile.mkdtemp(prefix="uniprot_swissprot_")
            self._annotations_file = os.path.join(self._temp_dir, "annotations.jsonl")
            self._features_file = os.path.join(self._temp_dir, "features.jsonl")
        ...
        # In low_memory_mode, annotations and features are written to temp files instead
    ...
    def cleanup(self):
        """Clean up temp files and file handles."""
        if self._annotations_file_handle:
            try:
                self._annotations_file_handle.close()
            except Exception:
                pass
            self._annotations_file_handle = None
            
        if self._features_file_handle:
            try:
                self._features_file_handle.close()
            except Exception:
                pass
            self._features_file_handle = None
    ...
    def _load_json_data(self):
        ...
        if self.low_memory_mode:
            logger.info("Low memory mode: annotations and features will be streamed to disk")
        t0 = time()
        ...
        # Open temp files for writing in low memory mode
        if self.low_memory_mode:
            self._annotations_file_handle = open(self._annotations_file, 'w', encoding='utf-8')
            self._features_file_handle = open(self._features_file, 'w', encoding='utf-8')
        ...
        for entry in tqdm(self._stream_entries(), desc="Loading JSON entries"):
            ...
            # Periodic memory cleanup in low memory mode
            if self.low_memory_mode and entry_count % 50000 == 0:
                gc.collect()
                logger.info(f"Processed {entry_count} entries, running garbage collection...")

        # Close temp file handles after processing
        if self.low_memory_mode:
            if self._annotations_file_handle:
                self._annotations_file_handle.close()
                self._annotations_file_handle = None
    ...
    def _extract_annotations(self, protein_id: str, entry: dict):
        ...
        if annotations:
            if self.low_memory_mode and self._annotations_file_handle:
                # Write to temp file in low memory mode
                record = {"protein_id": protein_id, "annotations": annotations}
                self._annotations_file_handle.write(json.dumps(record) + "\n")
            else:
    ...
    def _extract_features(self, protein_id: str, entry: dict):
        ...
        if feature_data:
            if self.low_memory_mode and self._features_file_handle:
                # Write to temp file in low memory mode
                record = {"protein_id": protein_id, "features": feature_data}
                self._features_file_handle.write(json.dumps(record) + "\n")
            else:
    ...
    def get_annotation_edges(
        ...
        if self.low_memory_mode and self._annotations_file and os.path.exists(self._annotations_file):
            # Stream from temp file in low memory mode
            logger.info(f"Streaming annotations from temp file: {self._annotations_file}")
    ...
    def get_feature_edges(
        ...
        if self.low_memory_mode and self._features_file and os.path.exists(self._features_file):
            # Stream from temp file in low memory mode
            logger.info(f"Streaming features from temp file: {self._features_file}")
